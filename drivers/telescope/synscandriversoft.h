...
class SynscanSoftDriver : public INDI::Telescope, public INDI::GuiderInterface {
public:
    SynscanSoftDriver();
    ~SynscanSoftDriver();
...
protected:
    ...
    virtual void processNSWE(double mag, double angle) override;

private:
    /**
     * @brief sendCommand Send a string command to mount.
     * @param cmd Command to be sent
     * @param cmd_len length of command in bytes. If not specified it is treated
     * as null-terminating string
     * @param res If not nullptr, the function will read until it detects the
     * default delimeter ('#') up to SYN_RES length. if nullptr, no read back is
     * done and the function returns true.
     * @param res_len length of buffer to read. If not specified, the buffer will
     * be read until the delimeter is met. if specified, only res_len bytes are
     * ready.
    ...
    static void cmdTimeout(void *context);
    bool cmdTimeoutThis();

    void calcOffset(double ra, double dec);

    std::tuple<double, double> ra_dec_to_alt_az(double ra, double dec) const;

    Eigen::Vector2d m_currentJoystick, m_prevJoystick;
    double m_azOffset{0}, m_altOffset{0};
    ...
    static constexpr uint16_t SIM_SLEW_RATE[] = {1,   8,   16,  32,  64,
                                                 128, 400, 600, 800, 900
                                                };
