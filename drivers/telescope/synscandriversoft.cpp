...
#include "indicom.h"
#include "libastro.h"
#include "synscandriversoft.h"

#include <libnova/precession.h>
#include <libnova/transform.h>
// libnova specifies round() on old systems and it collides with the new
// gcc 5.x/6.x headers
#define HAVE_ROUND
...
constexpr uint16_t SynscanSoftDriver::SIM_SLEW_RATE[];

SynscanSoftDriver::SynscanSoftDriver() {
    setVersion(2, 0);
...
const char *SynscanSoftDriver::getDefaultName() {
    return "SynScan Soft";
}

bool SynscanSoftDriver::initProperties() {
    INDI::Telescope::initProperties();

    SetTelescopeCapability(
        TELESCOPE_CAN_PARK | TELESCOPE_CAN_ABORT | TELESCOPE_CAN_SYNC |
        TELESCOPE_CAN_GOTO | TELESCOPE_HAS_TIME | TELESCOPE_HAS_LOCATION |
    ...
    /// Custom Slew Rate
    //////////////////////////////////////////////////////////////////////////////////////////////////
    IUFillNumber(&CustomSlewRateN[AXIS_RA], "AXIS1", "RA/AZ (arcsecs/s)", "%.2f",
                 0.05, 800, 10, 0);
    IUFillNumber(&CustomSlewRateN[AXIS_DE], "AXIS2", "DE/AL (arcsecs/s)", "%.2f",
                 0.05, 800, 10, 0);
    IUFillNumberVector(&CustomSlewRateNP, CustomSlewRateN, 2, getDeviceName(),
                       "CUSTOM_SLEW_RATE", "Custom Slew", MOTION_TAB, IP_RW, 60,
                       IPS_IDLE);

    //////////////////////////////////////////////////////////////////////////////////////////////////
    /// Guide Rate
    //////////////////////////////////////////////////////////////////////////////////////////////////
    IUFillNumber(&GuideRateN[AXIS_RA], "GUIDE_RATE_WE", "W/E Rate", "%.2f", 0, 1,
                 0.1, 0.5);
    IUFillNumber(&GuideRateN[AXIS_DE], "GUIDE_RATE_NS", "N/S Rate", "%.2f", 0, 1,
                 0.1, 0.5);
    IUFillNumberVector(&GuideRateNP, GuideRateN, 2, getDeviceName(), "GUIDE_RATE",
                       "Guiding Rate", GUIDE_TAB, IP_RW, 0, IPS_IDLE);

    //////////////////////////////////////////////////////////////////////////////////////////////////
    /// Horizontal Coords
    //////////////////////////////////////////////////////////////////////////////////////////////////
    IUFillNumber(&HorizontalCoordsN[AXIS_AZ], "AZ", "Az D:M:S", "%10.6m", 0.0,
                 360.0, 0.0, 0);
    IUFillNumber(&HorizontalCoordsN[AXIS_ALT], "ALT", "Alt  D:M:S", "%10.6m",
                 -90., 90.0, 0.0, 0);
    IUFillNumberVector(&HorizontalCoordsNP, HorizontalCoordsN, 2, getDeviceName(),
                       "HORIZONTAL_COORD", "Horizontal Coord", MAIN_CONTROL_TAB,
                       IP_RW, 0, IPS_IDLE);
    ...
    IUFillSwitchVector(&GotoModeSP, GotoModeS, NARRAY(GotoModeS), getDeviceName(),
                       "GOTOMODE", "Goto mode", MAIN_CONTROL_TAB, IP_RW,
...
bool SynscanSoftDriver::updateProperties() {
...
void SynscanSoftDriver::setupParams() {
    readFirmware();
    // readModel();
    readTracking();
...
int SynscanSoftDriver::hexStrToInteger(const std::string &res) {
    int result = 0;
...
bool SynscanSoftDriver::Handshake() {
    char res[SYN_RES] = {0};
    if (!echo())
        return false;
    ...
    if (res[0] == 0) {
        LOG_ERROR("Mount is not aligned. Please align the mount first and connect "
                  "again.");
        return false;
    }
...
bool SynscanSoftDriver::ISNewNumber(const char *dev, const char *name,
    ...
    if (dev && !strcmp(dev, getDeviceName())) {
        ...
        if (!strcmp(name, HorizontalCoordsNP.name)) {
            ...
            IDSetNumber(&HorizontalCoordsNP,
...
bool SynscanSoftDriver::ISNewSwitch(const char *dev, const char *name,
...
bool SynscanSoftDriver::echo() {
    char res[SYN_RES] = {0};
    return sendCommand("Kx", res);
}

bool SynscanSoftDriver::readFirmware() {
    ...
    if (sendCommand("V", res)) {
        ...
        if (m_FirmwareVersion < 3.38 ||
                (m_FirmwareVersion >= 4.0 && m_FirmwareVersion < 4.38)) {
...
bool SynscanSoftDriver::readTracking() {
    // Read the handset version
    char res[SYN_RES] = {0};
...
bool SynscanSoftDriver::readModel() {
    // extended list of mounts
    std::map<int, std::string> models = {
        {0, "EQ6 GOTO Series"},         {1, "HEQ5 GOTO Series"},
        {2, "EQ5 GOTO Series"},         {3, "EQ3 GOTO Series"},
        {4, "EQ8 GOTO Series"},         {5, "AZ-EQ6 GOTO Series"},
        {6, "AZ-EQ5 GOTO Series"},      {160, "AllView GOTO Series"},
        {161, "Virtuoso Alt/Az mount"}, {165, "AZ-GTi GOTO Series"}
    };
    ...
    LOGF_INFO("Driver is running in %s mode.",
              m_isAltAz ? "Alt-Az" : "Equatorial");
    LOGF_INFO("Detected mount: %s. Mount must be aligned from the handcontroller "
...
bool SynscanSoftDriver::ReadScopeStatus() {
    if (isSimulation()) {
        mountSim();
        return true;
    }
    ...
    // Synscan reports J2000 coordinates so we need to convert from J2000 to
    JNow LibAstro::J2000toObserved(&J2000Pos, ln_get_julian_from_sys(),
    &epochPos);
    ...
    az = range360(az + m_azOffset);
    al = rangeDec(range360(al + m_altOffset));
    ...
    auto lat = LocationN[LOCATION_LATITUDE].value;
    auto lng = LocationN[LOCATION_LONGITUDE].value;
    auto lst = get_local_sidereal_time(lng);
    // auto haDeg = get_local_hour_angle(lst, ra) * 360 / 24;
    double ha;
    double CurrentDE;

    get_ha_dec_coordinates(al, az, lat, &ha, &CurrentDE);

    ha *= 24 / 360.0;

    LOGF_DEBUG("HA %g", ha);

    double CurrentRA = range24(lst - ha);
...
bool SynscanSoftDriver::SetTrackEnabled(bool enabled) {
    ...
    sendCommand(cmd, res, 2);

    if (enabled) {
        auto ra = EqN[AXIS_RA].value;
        auto dec = EqN[AXIS_DE].value;
        auto raNextSec = ra + 1.0 / (60 * 60);
        double alt, az;
        double altNextSec, azNextSec;

        std::tie(alt, az) = ra_dec_to_alt_az(ra, dec);
        std::tie(altNextSec, azNextSec) = ra_dec_to_alt_az(raNextSec, dec);

        auto rangeArcseconds = [](double d) {
            d = range360(d);
        ...
        auto altRate = rangeArcseconds(altNextSec - alt);
        auto azRate = rangeArcseconds(azNextSec - az);

        LOGF_INFO("azRate %g, altRate %g", azRate, altRate);

        if (altRate > 0)
            slewVariableRate(SYN_N, altRate);
        else if (altRate < 0)
            slewVariableRate(SYN_S, -altRate);
        else
            slewFixedRate(SYN_N, 0);

        if (azRate > 0)
            slewVariableRate(SYN_W, azRate);
        else if (azRate < 0)
            slewVariableRate(SYN_E, -azRate);
        else
            slewFixedRate(SYN_E, 0);
    } else {
        slewFixedRate(SYN_N, 0);
        slewFixedRate(SYN_E, 0);
    }
...
bool SynscanSoftDriver::SetTrackMode(uint8_t mode) {
    char cmd[SYN_RES] = {0}, res[SYN_RES] = {0};
...
bool SynscanSoftDriver::SetAltAzMode(bool enable) {
...
bool SynscanSoftDriver::Goto(double ra, double dec) {
    char cmd[SYN_RES] = {0}, res[SYN_RES] = {0};
    ...
    if (goto_AltAz && m_isAltAz) // only if enabled to use AltAz goto
    {
        ...
        get_hrz_from_equ(&epochPos, &lnobserver, ln_get_julian_from_sys(),
                         &lnaltaz);
    ...
    LOGF_DEBUG("Goto - JNow RA: %g JNow DE: %g J2000 RA: %g J2000 DE: %g", ra,
               dec, J2000Pos.ra / 15.0, J2000Pos.dec);
...
bool SynscanSoftDriver::GotoAzAlt(double az, double alt) {
    char cmd[SYN_RES] = {0}, res[SYN_RES] = {0};
    ...
    if (m_isAltAz == false) {
        ...
        get_equ_from_hrz(&horizontalPos, &observer, ln_get_julian_from_sys(),
                         &equatorialPos);
...
bool SynscanSoftDriver::SetCurrentPark() {
    char res[SYN_RES] = {0};
    ...
    LOGF_DEBUG(
        "Setting current parking position to coordinates Az (%s) Alt (%s)...",
        AzStr, AltStr);
...
bool SynscanSoftDriver::SetDefaultPark() {
...
bool SynscanSoftDriver::MoveNS(INDI_DIR_NS dir,
    ...
    switch (command) {
    case MOTION_START:
        rc = (rate < 10) ? slewFixedRate(move, rate)
             : slewVariableRate(move, customRate);
        if (!rc) {
...
bool SynscanSoftDriver::MoveWE(INDI_DIR_WE dir,
                               TelescopeMotionCommand command) {
    if (isSimulation())
        return true;
...
bool SynscanSoftDriver::SetSlewRate(int s) {
...
bool SynscanSoftDriver::sendTime() {
    ...
    if (sendCommand("h", res)) {
        ...
        //  this is the daylight savings flag in the hand controller, needed if we
        ...
        sprintf(utc, "%04d-%02d-%dT%d:%02d:%02d", utcTime.years, utcTime.months,
                utcTime.days, utcTime.hours, utcTime.minutes, sec);
        if (daylightflag == 1)
            offset = offset + 1;
...
bool SynscanSoftDriver::sendLocation() {
    char res[SYN_RES] = {0};
...
bool SynscanSoftDriver::updateTime(ln_date *utc, double utc_offset) {
    char cmd[SYN_RES] = {0}, res[SYN_RES] = {0};
    ...
    cmd[7] = utc_offset > 0 ? static_cast<uint8_t>(utc_offset)
             : static_cast<uint8_t>(256 + utc_offset);
    ...
    LOGF_INFO(
        ...
        ltm.years, ltm.months, ltm.days, ltm.hours, ltm.minutes,
...
bool SynscanSoftDriver::updateLocation(double latitude, double longitude,
                                       double elevation) {
    ...
    LOGF_INFO("Update location to latitude %d:%d:%1.2f longitude %d:%d:%1.2f",
              p2.lat.degrees, p2.lat.minutes, p2.lat.seconds, p2.lng.degrees,
              p2.lng.minutes, p2.lng.seconds);
...
SynscanSoftDriver::ra_dec_to_alt_az(double ra, double dec) const {
    auto lng = LocationN[LOCATION_LONGITUDE].value;
    auto lat = LocationN[LOCATION_LATITUDE].value;
    auto lst = get_local_sidereal_time(lng);
    auto haDeg = get_local_hour_angle(lst, ra) * 360 / 24;
    ...
    get_alt_az_coordinates(haDeg, dec, lat, &alt, &az);

    return {alt, az};
}

bool SynscanSoftDriver::Sync(double ra, double dec) {
    TargetRA = ra;
    TargetDE = dec;
    ...
    double azScope = HorizontalCoordsN[AXIS_AZ].value;
    double altScope = HorizontalCoordsN[AXIS_ALT].value;

    azScope = range360(azScope - m_azOffset);
    altScope = rangeDec(range360(altScope - m_altOffset));

    LOGF_INFO("Should be at az %g, alt %g", az, alt);
    LOGF_INFO("Telescope at az %g, alt %g", azScope, altScope);

    m_azOffset = az - azScope;
    m_altOffset = alt - altScope;

    HorizontalCoordsN[AXIS_AZ].value = az;
    HorizontalCoordsN[AXIS_ALT].value = alt;
...
ln_hrz_posn SynscanSoftDriver::getAltAzPosition(double ra, double dec) {
...
void SynscanSoftDriver::sendStatus() {
    bool BasicMountInfoHasChanged = false;

    if (std::string(StatusT[MI_GOTO_STATUS].text) !=
            m_MountInfo[MI_GOTO_STATUS]) {
        IUSaveText(&StatusT[MI_GOTO_STATUS], m_MountInfo[MI_GOTO_STATUS].c_str());
        BasicMountInfoHasChanged = true;
    }
    if (std::string(StatusT[MI_POINT_STATUS].text) !=
            m_MountInfo[MI_POINT_STATUS]) {
...
bool SynscanSoftDriver::sendCommand(const char *cmd, char *res, int cmd_len,
                                    int res_len) {
    ...
    if (res_len > 0)
        ...
        rc = tty_nread_section(PortFD, res, SYN_RES, SYN_DEL, SYN_TIMEOUT,
                               &nbytes_read);
...
void SynscanSoftDriver::hexDump(char *buf, const char *data, int size) {
...
void SynscanSoftDriver::mountSim() {
    ...
    ltv = tv;
    double currentSlewRate = SIM_SLEW_RATE[IUFindOnSwitchIndex(&SlewRateSP)] *
                             TRACKRATE_SIDEREAL / 3600.0;
    da = currentSlewRate * dt;

    /* Process per current state. We check the state of EQUATORIAL_COORDS and act
     * acoordingly */
    switch (TrackState) {
    case SCOPE_IDLE:
...
bool SynscanSoftDriver::slewFixedRate(SynscanDirection direction,
                                      uint8_t rate) {
    char cmd[SYN_RES] = {0}, res[SYN_RES] = {0};
...
bool SynscanSoftDriver::slewVariableRate(SynscanDirection direction,
        double rate) {
    char cmd[SYN_RES] = {0}, res[SYN_RES] = {0};
    ...
    if (!m_isAltAz)
        ...
        cmd[3] = (direction == SYN_N || direction == SYN_W) ? 7 : 6;
    // High byte
    cmd[4] = synRate >> 8;
    // Low byte
    cmd[5] = synRate & 0xFF;
...
IPState SynscanSoftDriver::GuideNorth(uint32_t ms) {
    if (m_GuideNSTID) {
    ...
    m_CustomGuideDE =
        TRACKRATE_SIDEREAL + GuideRateN[AXIS_DE].value * TRACKRATE_SIDEREAL;
    MoveNS(DIRECTION_NORTH, MOTION_START);
    m_GuideNSTID = IEAddTimer(ms, guideTimeoutHelperNS, this);
    return IPS_BUSY;
}

IPState SynscanSoftDriver::GuideSouth(uint32_t ms) {
    if (m_GuideNSTID) {
        IERmTimer(m_GuideNSTID);
        m_GuideNSTID = 0;
    }
...
IPState SynscanSoftDriver::GuideEast(uint32_t ms) {
    if (m_GuideWETID) {
        IERmTimer(m_GuideWETID);
        m_GuideWETID = 0;
    }

    // So if we SID_RATE + 0.5 * SID_RATE for example, that's 150% of sidereal
    // rate but for east we'd be going a lot faster since the stars are moving
    // toward the west in sideral rate. Just standing still we would SID_RATE
    // moving across. So for east we just go GuideRate * SID_RATE without adding
    // any more values.
    // m_CustomGuideRA = TRACKRATE_SIDEREAL + GuideRateN[AXIS_RA].value *
    // TRACKRATE_SIDEREAL;
...
IPState SynscanSoftDriver::GuideWest(uint32_t ms) {
    if (m_GuideWETID) {
        IERmTimer(m_GuideWETID);
        m_GuideWETID = 0;
    }
    ...
    m_CustomGuideRA =
        TRACKRATE_SIDEREAL + GuideRateN[AXIS_RA].value * TRACKRATE_SIDEREAL;
    MoveWE(DIRECTION_WEST, MOTION_START);
    m_GuideWETID = IEAddTimer(ms, guideTimeoutHelperWE, this);
    return IPS_BUSY;
}

void SynscanSoftDriver::guideTimeoutHelperNS(void *context) {
    static_cast<SynscanSoftDriver *>(context)->guideTimeoutCallbackNS();
}

void SynscanSoftDriver::guideTimeoutHelperWE(void *context) {
    static_cast<SynscanSoftDriver *>(context)->guideTimeoutCallbackWE();
}

void SynscanSoftDriver::guideTimeoutCallbackNS() {
    INDI_DIR_NS direction =
        static_cast<INDI_DIR_NS>(IUFindOnSwitchIndex(&MovementNSSP));
    MoveNS(direction, MOTION_STOP);
    GuideComplete(AXIS_DE);
    m_CustomGuideDE = m_GuideNSTID = 0;
}

void SynscanSoftDriver::guideTimeoutCallbackWE() {
    INDI_DIR_WE direction =
        static_cast<INDI_DIR_WE>(IUFindOnSwitchIndex(&MovementWESP));
    MoveWE(direction, MOTION_STOP);
    GuideComplete(AXIS_RA);
    m_CustomGuideRA = m_GuideWETID = 0;
}

bool SynscanSoftDriver::isSlewComplete() {
    char res[SYN_RES] = {0};
...
void SynscanSoftDriver::processNSWE(double mag, double angle) {
    // mag = std::pow(mag, 1 / 2.f);

    Eigen::Vector2d v(cos(angle * M_PI / 180) * mag,
                      -sin(angle * M_PI / 180) * mag);

    Eigen::Vector2d vmax(cos(angle * M_PI / 180), -sin(angle * M_PI / 180));

    vmax = vmax.cwiseAbs();
    vmax /= std::max(vmax[0], vmax[1]);
    v *= vmax.norm();
    ...
    /*for (int i = 0; i < 2; ++i) {
        ...
        auto r = std::floor(cr * (maxRate + 1 + .5));

        if (r <= minRate)
            r = 0;

        v[i] = std::copysign(r, v[i]);
    }*/

    m_currentJoystick = v;

    // Allow pending joystick messages to unqueue before sending message
    if (m_cmdTID==0)
        m_cmdTID =IEAddTimer(10, cmdTimeout, this);
}

void SynscanSoftDriver::cmdTimeout(void *context) {
    auto self = static_cast<SynscanSoftDriver *>(context);

    self->m_cmdTID = 0;
    self->cmdTimeoutThis();
}

bool SynscanSoftDriver::cmdTimeoutThis() {
    Eigen::Vector2d &v = m_currentJoystick;
    Eigen::Vector2d &prev = m_prevJoystick;

    bool commanded = false;

    auto move = [&](int axis, bool positive, TelescopeMotionCommand cmd) {
        commanded = true;

        if (axis == 0) {
            auto dir = !positive ? DIRECTION_WEST : DIRECTION_EAST;

            LOGF_INFO("MoveWE(%d, %d)", dir, cmd);
            MoveWE(dir, cmd);
        } else {
            auto dir = positive ? DIRECTION_NORTH : DIRECTION_SOUTH;

            LOGF_INFO("MoveNS(%d, %d)", dir, cmd);
            MoveNS(dir, cmd);
        }
    };

    auto getDirection = [] (int axis, double value) {
        SynscanDirection dir;

        if (axis==0) dir = value>=0 ? SYN_E : SYN_W;
        else dir = value>=0 ? SYN_N : SYN_S;

        return dir;
    };

    auto getSlewRate = [] (double value) {
        const double rates[] = { 1,8,16,32,64,128,400,600,800 /* max guess */};
        const int ratesNum = sizeof(rates)/sizeof(rates[0]); // or use std::size in C++17
        double rate = std::abs(value)*rates[ratesNum-1];
        double maxdelta = std::numeric_limits<double>::infinity();
        int rateIdx = 0;

        for (int i=0; i<ratesNum; ++i)
        {
            double delta = std::abs(rate-rates[i]);
    ...
    auto slew = [&](int axis, double previous, double current) {
        if (previous==current) return;

        SynscanDirection dir = getDirection(axis, current);
        SynscanDirection dirPrev = getDirection(axis, previous);

        if (current==0) {
            //return move(axis, previous>=0, MOTION_STOP);
            slewFixedRate(dirPrev, 0);

            return;
        }

#ifdef USE_VARIABLE_RATE
        const double maxRate = 0xffff/4.0;
        bool same = rint(current*maxRate*4)==rint(previous*maxRate*4);

        if (same) return;

        slewVariableRate(dir, std::abs(current*maxRate));
#else
        int rateIdx = getSlewRate(current);
        int rateIdxPrevious = getSlewRate(previous);

        if (rateIdx==rateIdxPrevious) return;

        slewFixedRate(dir, rateIdx);
        //move(axis, current>=0, MOTION_START);
#endif
    };

    LOGF_DEBUG("v(%f, %f)", v[0], v[1]);
    // LOGF_DEBUG("prev(%f, %f)", prev[0], prev[1]);
...
void SynscanSoftDriver::calcOffset(double ra, double dec) {
    auto lng = LocationN[LOCATION_LONGITUDE].value;
    auto lat = LocationN[LOCATION_LATITUDE].value;
    auto lst = get_local_sidereal_time(lng);
    auto haDeg = get_local_hour_angle(lst, ra) * 360 / 24;
